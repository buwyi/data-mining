    ...
    public PySession create() throws Exception {
        ...
        while (flag && attempts > 0) {
            ...
            try {
                ...
                // macOS 等环境常无 python 仅有 python3；老环境可配置为 python
                String pythonCmd = configuration.getString(server + "[@pythoncmd]", "python3");
                ...
                pySession = new PySession(session, pythonCmd);
                if (null != pySession) {
                    flag = false;
                }
            } catch (Exception ex) {
